...
def run_game(black_player, white_player, game_num, total_games, sgf_dir):
    ...
    print(
        f"Black: {black_player.name} ({black_player.elo}), White: {white_player.name} ({white_player.elo})"
    )
    ...
    try:
        ...
        for turn in range(1, config.BOARD_SIZE * config.BOARD_SIZE * 2 + 1):
            ...
            if move_str == "pass":
                ...
                try:
                    ...
                    if not (
                        0 <= col < config.BOARD_SIZE and 0 <= row < config.BOARD_SIZE
                    ):
        ...
        # Save SGF file
        sgf_filename = os.path.join(
            sgf_dir,
            f"{black_player.name}_vs_{white_player.name}_{int(time.time())}.sgf",
        )
        try:
...
def main():
    ...
    parser.add_argument(
        "--games", type=int, default=2, help="Number of games per matchup."
    )
    parser.add_argument(
        "--sgf_dir", type=str, default="sgf_games", help="Directory to save SGF files."
    )
    ...
    if not args.player or len(args.player) < 2:
        print(
            "Please specify at least two players using --player NAME CMD",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    print(
        f"Matchups: {len(matchups)}, Games per matchup: {args.games}, Total games: {total_games}"
    )
    ...
    for p1, p2 in matchups:
        for i in range(args.games):
            ...
            winner = run_game(
                black_player, white_player, game_count, total_games, args.sgf_dir
            )

            if winner == "B":
                print(f"Result: Black ({black_player.name}) wins.")
                black_score, _ = 1.0, 0.0
            elif winner == "W":
                print(f"Result: White ({white_player.name}) wins.")
                black_score, _ = 0.0, 1.0
            else:  # Draw
                print("Result: Draw.")
                black_score, _ = 0.5, 0.5

            # Update Elo
            b_elo, w_elo = black_player.elo, white_player.elo
            black_player.elo, white_player.elo = update_elo(b_elo, w_elo, black_score)
            print(
                f"New Ratings: {black_player.name} = {black_player.elo}, {white_player.name} = {white_player.elo}"
            )
